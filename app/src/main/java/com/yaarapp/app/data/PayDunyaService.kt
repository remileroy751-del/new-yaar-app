...
class PayDunyaService {
    ...
    suspend fun createPayment(request: PayDunyaCreateRequest): PayDunyaCreateResponse =
        client.functions.invoke("paydunya-create-invoice", body = request).body<PayDunyaCreateResponse>()

    suspend fun checkPayment(paymentId: String): PayDunyaCheckResponse =
        client.functions.invoke("paydunya-check-payment", body = PayDunyaCheckRequest(paymentId)).body<PayDunyaCheckResponse>()
}
